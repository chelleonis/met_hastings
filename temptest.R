...
wow <- function(params) {
   hi <- dgamma(params[1],params[2], log = TRUE)
   return(hi)
 }
# 
# mc_og <- Metro_Hastings(li_func = wow, pars = c(1.7,4.4),
#                par_names = c('a','b'), iterations = 10000, burn_in = 1000)
...
mc_new <- met_hastings(10000, start = 1, burn_in = 1000, jparams = 2,
                distr = "gamma",dparams = c(1.7,4.4))
bayes_CI(mc_new, 0.1)